...
public class Spiral<Cell> implements Sequence<Cell> {
	...
	public Spiral(Grid2D<Cell, ?> grid, Cell start) {
		...
		Grid2D<Integer, DefaultEdge<Integer>> quadraticGrid = new RawGrid(size, size);
		...
		for (int i = 0, n = size / 2 + 1; i < n; ++i) {
			...
			if (i < n - 1) {
				leftUpperCorner = quadraticGrid.cell(quadraticGrid.col(leftUpperCorner) - 1, quadraticGrid.row(leftUpperCorner) - 1);
			}
		}
	}
