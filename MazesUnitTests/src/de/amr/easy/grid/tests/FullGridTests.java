	...
	public void testConnectedTowards() {
		for (int x = 0; x < grid.numCols(); ++x) {
			for (int y = 0; y < grid.numRows(); ++y) {
				...
				if (grid.numCols() > 1) {
					if (x == 0) {
						assertTrue(grid.isConnected(cell, Direction4.E));
					}
					if (x == grid.numCols() - 1) {
						assertTrue(grid.isConnected(cell, Direction4.W));
					}
				}
				if (grid.numRows() > 1) {
					if (y == 0) {
						assertTrue(grid.isConnected(cell, Direction4.S));
					}
					if (y == grid.numRows() - 1) {
						assertTrue(grid.isConnected(cell, Direction4.N));
					}
				}
			}
		}
